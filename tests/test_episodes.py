...
def test_filter_episodes():
    response = client.get("/episodes/filter")
    assert response.status_code == 200
    assert len(response.json()) == 20
    assert response.json()[0]["name"] == "Pilot"

    response = client.get("/episodes/filter?name=rixty%20minutes")
    assert response.status_code == 200
...
def test_character_count_sorted():
    response = client.get("/episodes/character-count-sorted")
    assert response.status_code == 200
    assert len(response.json()) == 20
