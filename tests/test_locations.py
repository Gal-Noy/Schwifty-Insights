...
def test_filter_locations():
    ...
    assert response.json()["data"][0]["name"] == "Earth (C-137)"
    ...
    assert response.status_code == 200
    assert len(response.json()["data"]) == 20
    assert response.json()["data"][0]["name"] == "Testicle Monster Dimension"
...
def test_residents_sorted():
    ...
    assert response.status_code == 200
    assert len(response.json()["data"]) == 20
    assert response.json()["data"][0]["name"] == "Earth (Replacement Dimension)"
    assert response.json()["data"][0]["resident_count"] == 230

    response = client.get("/locations/residents-sorted?page=2", headers={"Authorization": f"Bearer {BEARER_TOKEN}"})
    assert response.status_code == 200
    assert len(response.json()["data"]) == 20
    assert response.json()["data"][0]["name"] == "Unity's Planet"
    assert response.json()["data"][0]["resident_count"] == 6
...
def test_type_sorted():
    ...
    assert response.status_code == 200
    assert len(response.json()["data"]) == 20
    assert response.json()["data"][0]["name"] == "Signus 5 Expanse"
    assert response.json()["data"][0]["type"] == "unknown"

    response = client.get("/locations/type-sorted?page=2", headers={"Authorization": f"Bearer {BEARER_TOKEN}"})
    assert response.status_code == 200
    assert len(response.json()["data"]) == 20
    assert response.json()["data"][0]["name"] == "Z. Q. P. D."
    assert response.json()["data"][0]["type"] == "Police Department"
